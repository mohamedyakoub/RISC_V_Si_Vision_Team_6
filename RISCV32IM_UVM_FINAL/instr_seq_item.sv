...
// Hazard control
  rand bit        generate_raw_hazard;
  rand bit        generate_waw_hazard;
  rand bit        generate_war_hazard;
  
  // Previous instruction tracking
  bit [4:0]      prev_rd;        // Previous destination register
  bit            prev_rd_valid;  // Previous instruction wrote to reg
  bit [4:0]      prev_rs1;       // Previous source register 1
  bit [4:0]      prev_rs2;       // Previous source register 2
  bit            prev_is_load;   // Previous instruction was load
  bit		 reg_order;
...
// RAW hazard constraint
  constraint raw_hazard {
    if (generate_raw_hazard && prev_rd_valid) {
      // Current instruction must read from register written by previous
      if (balance == 0) { 
          (opcode inside {R_TYPE, I_TYPE_0, I_TYPE_1}) -> rs1 == prev_rd;
      } else {
      
          (opcode inside {R_TYPE, I_TYPE_0, I_TYPE_1}) -> rs2 == prev_rd;
      }
  ...
  // WAW hazard constraint
  constraint waw_hazard {
    if (generate_waw_hazard && prev_rd_valid) {
      // Current instruction writes to same register as previous
      opcode inside {R_TYPE, I_TYPE_0, I_TYPE_1};
      rd == prev_rd;
   
    }
  }

  // WAR hazard constraint
  constraint war_hazard {
    if (generate_war_hazard) {
      // Current instruction writes to register read by previous
      opcode inside {R_TYPE, I_TYPE_0, I_TYPE_1};
      // Check previous instruction's source registers
      (prev_rs1 != 0) -> rd == prev_rs1;
      (prev_rs2 != 0) -> rd == prev_rs2;
   
    }
  }

  // Hazard mutual exclusion
  constraint hazard_exclusion {
    generate_raw_hazard + generate_waw_hazard + generate_war_hazard <= 1;
  }

  // Control probability of hazard generation
  constraint hazard_control {
    generate_raw_hazard dist { 1 := 80, 0 := 20 };
    generate_waw_hazard dist { 1 := 20, 0 := 80 };
    generate_war_hazard dist { 1 := 30, 0 := 70 };
  }   

function void post_randomize();

    	      prev_rd = rd;
              prev_rd_valid = (opcode inside {R_TYPE, I_TYPE_0, I_TYPE_1}) && (rd != 0);
              prev_rs1 = rs1;
              prev_rs2 = rs2;
              prev_is_load = (opcode == LW);
	      //if (reg_order==1) begin rd==prev_rd+1; end
