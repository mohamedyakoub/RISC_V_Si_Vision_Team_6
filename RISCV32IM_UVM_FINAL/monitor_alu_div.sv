...
//////////////   RUN PHASE   //////////////////////////////
	...
	seq_collected.rst_n               = vif.rst_n;
        ...
        if (vif.operator_i == ALU_DIVU || vif.operator_i == ALU_DIV || vif.operator_i == ALU_REMU|| vif.operator_i == ALU_REM )begin
          //@(posedge vif.clk iff (vif.ex_ready_i && vif.ready_o));
            wait((vif.ex_ready_i && vif.ready_o)) ;
    ...
    //`uvm_info("ALU_DIV_Mon", $sformatf("Actual_result = %h A = %0h B = %0h opcode = %0s",seq_collected.result_o,seq_collected.operand_a_i,seq_collected.operand_b_i,seq_collected.operator_i.name()), UVM_HIGH)
