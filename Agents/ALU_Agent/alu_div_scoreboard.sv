...
class  alu_div_scoreboard extends uvm_scoreboard;
    ...
    uvm_analysis_imp_exp#(alu_div_seq_item,alu_div_scoreboard) exp_ap;
    uvm_analysis_imp_act#(alu_div_seq_item,alu_div_scoreboard) act_ap;
    ...
    virtual function  write_exp(alu_div_seq_item item);
        `uvm_info("alu_div_scoreboard", "Packet received", UVM_HIGH)
        expected.push_back(item);
        //item.print();
    endfunction

    virtual function  write_act(alu_div_seq_item item);
        `uvm_info("alu_div_scoreboard", "Packet received", UVM_HIGH)
    ...
    virtual task run_phase(uvm_phase phase);
        ...
        forever begin
            ...
            if(actual[0].result_o == expected[0].result_o) begin
		        ...
		        `uvm_info("alu_div_scoreboard", $sformatf("Expected: %0h, Actual: %0h", expected[0].result_o, actual[0].result_o), UVM_HIGH)
                correct=correct+1;
            end
            else begin
